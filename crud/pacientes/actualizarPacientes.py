...
class ActualizarPacientes(CrearPacientes):
    def __init__(self, parent, id_seleccionado, usuario={}):
        ...
        self.pa_nombre_contacto_emergencia.insert(
            0, self.pacientes.get("pa_nombre_contacto_emergencia", ""))
        self.id_enfermera_seleccionado = self.pacientes.get(
            "id_enfermera_principal")
        self.pa_tel_contacto_emergencia.insert(
            0, str(self.pacientes.get("pa_tel_contacto_emergencia", "")))
        self.combo_id_enfermera.current(
            obtener_indice(
                self.id_enfermera_seleccionado,
                self.enfermeras))
