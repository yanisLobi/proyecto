...
def _mezclar_color(hex_color, bg="#1e1e1e", factor=0.45):
    ...
    try:
        r1, g1, b1 = int(hex_color[1:3], 16), int(
...
class CalendarioRecordatorios(ttk.Frame):
    def __init__(self, parent, navegar_cb=None, usuario=None):
        ...
        altura_total = (self.HORA_FIN - self.HORA_INICIO + 1) * \
            self.PIXELS_POR_HORA
    ...
    def _hora_a_float(self, hora_str):
        try:
            ...
            return int(partes[0]) + (int(partes[1]) / \
                       60 if len(partes) >= 2 else int(partes[0]))
        except Exception:
            return None
    ...
    def _cargar_eventos_mongo(self):
        ...
        try:
            ...
            print(
                f"[Calendario] {
                    len(registros)} registros obtenidos de MongoDB")
        ...
        for i, reg in enumerate(registros):
            try:
                ...
                if ids_permitidos is not None and str(
                        reg.get("id_tr", "")) not in ids_permitidos:
                    continue
                ...
                if col_idx is None:
                    print(
                        f"[Calendario] Evento {i}: fecha '{fecha_str}' fuera de la semana actual")
                    continue
                ...
                titulo = (
                    reg.get("re_titulo") or "Sin título").strip('"').strip()
                ...
                if _mysql_get and id_tr:
                    try:
                        trat_l = _mysql_get(
                            "tratamientos", "id_tratamientos", id_tr, False)
                        if trat_l:
                            id_pac = trat_l[0].get("id_paciente")
                            if id_pac:
                                pac_l = _mysql_get(
                                    "pacientes", "id_pacientes", id_pac, False)
                                if pac_l:
                                    p = pac_l[0]
                                    nombre_paciente = f"{
                                        p.get(
    ...
    def dibujar_cuadricula(self, ancho_ventana):
        ...
        for i in range(8):
            ...
            altura_total = (
                self.HORA_FIN - self.HORA_INICIO + 1) * self.PIXELS_POR_HORA
            self.canvas.create_line(
                x, 0, x, altura_total, fill="#2d2d2d", width=1)
    ...
    def dibujar_eventos(self):
        ...
        for idx, (titulo, dia_idx, h_inicio, h_fin, color, id_evento,
            ...
            completado = self._eventos_raw[idx].get(
                "re_estado", "") == "Completado"
    ...
    def dibujar_indicador_tiempo(self):
        ...
        # 2. Calcular los límites X para que solo cubra la columna del día
        # actual
        x_inicial = self.MARGEN_IZQUIERDO + \
            (self.DIA_ACTUAL_INDEX * self.ancho_columna)
    ...
    def _filtrar_campos(self, datos, incluir_identificador=False):
        ...
        for nombre, valor in datos.items():
            ...
            if clave_lower in {"id", "_id"} or clave_lower.startswith(
                    "id_") or clave_lower.endswith("_id"):
                continue
    ...
    def _campo(self, parent, etiqueta, valor, bg):
        ...
        tk.Label(
            fila, text=str(valor) if valor not in (
                None, "", "—") else "—", fg="#e0e0e0", bg=bg, font=(
                "Arial", 8), anchor="w", wraplength=360, justify="left").pack(
    ...
    def _abrir_popup(self, idx):
        ...
        titulo, col_idx, h_inicio, h_fin, color, id_evento, id_tr, nombre_paciente = self.eventos[
            idx]
        ...
        if _mysql_get and id_tr:
            ...
            if id_paciente:
                try:
                    pac_list = _mysql_get(
                        "pacientes", "id_pacientes", id_paciente, False)
            ...
            if id_doctor:
                try:
                    doc_list = _mysql_get(
                        "usuarios", "id_usuarios", id_doctor, False)
            ...
            if id_enfermera:
                try:
                    enf_list = _mysql_get(
                        "usuarios", "id_usuarios", id_enfermera, False)
        ...
        sc_scroll = ttk.Scrollbar(
            popup, orient="vertical", command=sc_canvas.yview)
        ...
        inner.bind(
            "<Configure>",
            lambda e: sc_canvas.configure(
                scrollregion=sc_canvas.bbox("all")))
        sc_canvas.bind(
            "<Configure>",
            lambda e,
            wid=win_id: sc_canvas.itemconfig(
                wid,
                width=e.width))
        ...
        self._seccion(
            inner, "Evento", [
                ("Título", titulo), ("Fecha", raw.get(
                    "re_fecha", "—")), ("Inicio", raw.get(
                        "re_hora_inicio", "—")), ("Fin", raw.get(
                            "re_hora_fin", "—")), ("Estado", raw.get(
                                "re_estado", "—")), ("Observaciones", raw.get(
                                    "re_observaciones", "—")), ], BG_HDR, BG_BODY, link_cmd=(
                lambda iv=id_evento: self._navegar(
                    "mongo", "Actualizar eventos", id_seleccionado=iv)) if puede_editar_resto else None)
        ...
        if trat:
            self._seccion(
                ...
                self._filtrar_campos(
                    trat,
                    incluir_identificador=True),
                BG_HDR,
                BG_BODY,
                link_cmd=(
                    lambda it=id_trat: self._navegar(
                        "mysql",
                        "Actualizar tratamientos",
                        id_seleccionado=it) if it else None) if puede_editar_resto else None)
        else:
            self._seccion(
                inner, "Tratamiento", [
                    ("Detalle", "Sin tratamiento asociado")], BG_HDR, BG_BODY)
        ...
        if pac:
            self._seccion(
                ...
                BG_BODY,
                link_cmd=(
                    lambda ip=id_paciente: self._navegar(
                        "mysql",
                        "Actualizar pacientes",
                        id_seleccionado=ip) if ip else None) if puede_editar_resto else None)
        else:
            self._seccion(
                inner, "Paciente", [
                    ("Detalle", "Sin paciente asociado")], BG_HDR, BG_BODY)
        ...
        if doc:
            self._seccion(
                ...
                BG_BODY,
                link_cmd=(
                    lambda id_d=id_doctor: self._navegar(
                        "mysql",
                        "Actualizar usuarios",
                        id_seleccionado=id_d) if id_d else None) if puede_editar_usuarios else None)
        else:
            self._seccion(
                inner, "Doctor", [
                    ("Detalle", "Sin doctor asociado")], BG_HDR, BG_BODY)
        ...
        if enf:
            self._seccion(
                ...
                BG_BODY,
                link_cmd=(
                    lambda id_e=id_enfermera: self._navegar(
                        "mysql",
                        "Actualizar usuarios",
                        id_seleccionado=id_e) if id_e else None) if puede_editar_usuarios else None)
        else:
            self._seccion(
                inner, "Enfermera", [
                    ("Detalle", "Sin enfermera asociada")], BG_HDR, BG_BODY)
        ...
        if medicinas:
            for i, med in enumerate(medicinas, start=1):
                ...
                self._seccion(
                    ...
                    BG_BODY,
                    link_cmd=(
                        lambda im=id_med: self._navegar(
                            "mysql",
                            "Actualizar medicamentos",
                            id_seleccionado=im) if im else None) if puede_editar_resto else None)
        else:
            self._seccion(
                inner, "Medicinas", [
                    ("Detalle", "Sin medicinas asociadas al tratamiento")], BG_HDR, BG_BODY)
